    ...
    try {
      ...
      request = $.ajax({
        ...
        success    : function (responseText) {
                       // Store the path to the focused element (if applicable):
                       var focusedPaths = $("input :focus").map(function () {
                         return $(this).getPath();
                       });
                       
                       Smoke.log(focusedPaths);
                                              
                       eval(responseText);
                       
                       if (focusedPaths.length >= 1) {
                         $(focusedPaths[0]).focus();
                       }
