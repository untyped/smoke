             ...
             (lambda (name typed expected)
               (with-check-info (['sequence-name name])
                 (when typed
                   ...
                   (click/wait (node/id 'submit-button)))
                 (check-equal? (js-ref (js (!dot (!index ,(node/id 'text-area) 0) value)))
                               (or typed expected))
                 (check-equal? (inner-html-ref (node/id 'text-area-value)) expected)))])
        (check-equal? (js-ref (js (!dot (!index ,(node/id 'text-area) 0) rows))) 10)
        (check-equal? (js-ref (js (!dot (!index ,(node/id 'text-area) 0) cols))) 20)
        (do-checks "Initial" #f "Initial")
        (do-checks "Normal value" "Text" "Text")
        (do-checks "Blank" "" "no")
      ...
      (let ([do-checks ; string boolean -> void
             (lambda (name expected)
               (with-check-info (['sequence-name name])
                 ...
                 (click/wait (node/id 'submit-button))
                 (check-equal? (js-ref (js (!dot (!index ,(node/id 'check-box) 0) checked))) expected)
                 (check-equal? (inner-html-ref (node/id 'check-box-value)) (if expected "yes" "no"))))])
        (check-equal? (js-ref (js (!dot (!index ,(node/id 'check-box) 0) checked))) #t)
             ...
             (lambda (name selected expected)
               (with-check-info (['sequence-name name])
                 (when selected
                   ...
                   (click/wait (node/id 'submit-button))
                   (check-equal? (js-ref (js (!dot (!index ,(node/id selected) 0) checked))) #t))
                 (check-equal? (inner-html-ref (node/id 'radio-group-value)) expected)))])
        (check-equal? (js-ref (js (!dot (!index ,(node/id 'radio-button2) 0) checked))) #t)
